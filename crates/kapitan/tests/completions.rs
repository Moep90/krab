...
fn scratch(tag: &str) -> PathBuf {
    let dir =
        std::env::temp_dir().join(format!("kapitan-completions-{tag}-{}", std::process::id()));
...
fn registration_uses_the_invoked_name() {
    ...
    let link = dir.join("kapitan2");
    let _ = std::fs::remove_file(&link);
    std::os::unix::fs::symlink(env!("CARGO_BIN_EXE_kapitan"), &link).unwrap();
    ...
    for shell in ["bash", "zsh", "fish", "elvish", "powershell"] {
        ...
        assert!(
            !script.contains("kapitan "),
            "{shell}: script still registers `kapitan`:\n{script}"
        );
    }
...
fn registration_with_a_bare_name_leaves_lookup_to_path() {
    let exe = PathBuf::from(env!("CARGO_BIN_EXE_kapitan"));
    let out = Command::new(&exe)
        ...
        .unwrap();
    let script = String::from_utf8(out.stdout).unwrap();
    assert!(script.starts_with("#compdef kapitan\n"), "{script}");
}
