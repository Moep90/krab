...
fn main() -> ExitCode {
    let cli = Cli::parse();
    let foreground_server = matches!(cli.command, Command::Server { command: ServerCommand::Run { .. } });
    let filter = tracing_subscriber::EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| tracing_subscriber::EnvFilter::new(if foreground_server { "info" } else { "warn" }));
...
impl App {
    ...
    fn rpc_fail(&self, e: ClientError) -> Failure {
        match e {
            ClientError::Rpc(err) => {
                let diags: Vec<Diagnostic> = err.data.and_then(|d| serde_json::from_value(d).ok()).unwrap_or_default();
                if diags.is_empty() {
                    Failure::Diagnostics(vec![Diagnostic::error("server::error", err.message)], self.json)
                } else {
...
fn run(cli: Cli) -> Result<(), Failure> {
    ...
    let app = App { inv, json: cli.json, indent: dot.indent.unwrap_or(2), connector };
        ...
        Command::Inventory(args) => match args.command {
            ...
            Some(InventoryCommand::Classes { target }) => {
                let classes: Vec<String> = match app.client() {
                    Some(mut c) => c.call("inventory.classes", TargetParams { name: target, path: None }).map_err(|e| app.rpc_fail(e))?,
                    None => app.inv.render_named(&target).map_err(|e| app.fail(vec![e]))?.classes,
                };
                if app.json {
            ...
            Some(InventoryCommand::Explain { target, path }) => {
                let explanation: Explanation = match app.client() {
                    Some(mut c) => c.call("inventory.explain", ExplainParams { target, path }).map_err(|e| app.rpc_fail(e))?,
                    None => {
                        let t = app.inv.render_named(&target).map_err(|e| app.fail(vec![e]))?;
                        kapitan_inventory::explain::explain(&app.inv, &t, &path).map_err(|e| app.fail(vec![e]))?
                    }
                };
                if app.json {
            ...
            Some(InventoryCommand::Deps { files }) => {
                let names: Vec<String> = match app.client() {
                    Some(mut c) => c.call("inventory.deps", DepsParams { files }).map_err(|e| app.rpc_fail(e))?,
                    None => {
...
fn all_documents(app: &App) -> Result<Map, Failure> {
    ...
    match app.client() {
        Some(mut c) => {
            let all: AllResult = c.call("inventory.all", serde_json::Value::Null).map_err(|e| app.rpc_fail(e))?;
...
fn show(app: &App, args: ShowArgs) -> Result<(), Failure> {
    ...
    let doc = match &args.target {
        Some(name) => match app.client() {
            Some(mut c) => {
                let r: TargetResult = c
                    .call("inventory.target", TargetParams { name: name.clone(), path: args.pattern.clone() })
        ...
        None => select_pattern(app, Node::synthetic(Value::Map(all_documents(app)?)), args.pattern.as_deref())?,
    };
    let opts = DumpOptions { indent, ..DumpOptions::default() };
...
fn select_pattern(app: &App, doc: Node, pattern: Option<&str>) -> Result<Node, Failure> {
    let Some(pattern) = pattern else { return Ok(doc) };
...
fn targets(app: &App, quiet: bool) -> Result<(), Failure> {
    let summaries: Vec<TargetSummary> = match app.client() {
        Some(mut c) => c.call::<_, TargetsResult>("inventory.targets", serde_json::Value::Null).map_err(|e| app.rpc_fail(e))?.targets,
        None => app
            .inv
            .discover_targets()
            .map_err(|e| app.fail(vec![e]))?
            .into_iter()
            .map(|s| TargetSummary { name: s.name, path: s.path, file: s.file, digest: None, ok: true, error: None })
            .collect(),
    };
    if app.json {
...
fn check(app: &App) -> Result<(), Failure> {
    match app.client() {
        Some(mut c) => {
            let d: DiagnosticsResult = c.call("inventory.diagnostics", serde_json::Value::Null).map_err(|e| app.rpc_fail(e))?;
            let targets: TargetsResult = c.call("inventory.targets", serde_json::Value::Null).map_err(|e| app.rpc_fail(e))?;
            for w in &d.warnings {
                report::print_diagnostic(w, app.json);
            }
...
fn watch(app: &App, cwd: &Path) -> Result<(), Failure> {
    let Some(connector) = &app.connector else {
        return Err(Failure::Message("watch needs the inventory server; drop --no-daemon / --raw".into()));
    };
    ...
    loop {
        let r: WaitResult = client
            .call("inventory.wait", WaitParams { generation, timeout_ms: Some(60_000) })
            .map_err(|e| app.rpc_fail(e))?;
        for change in &r.changes {
            ...
            let failed = if change.errors.is_empty() { String::new() } else { format!(", {} failed", change.errors.len()) };
            eprintln!(
            ...
            if !shown.is_empty() {
                eprintln!("  targets: {}{}", shown.iter().map(|s| s.as_str()).collect::<Vec<_>>().join(", "), if change.rerendered.len() > 15 { ", …" } else { "" });
            }
...
fn rel(path: &Path, cwd: &Path) -> String {
    path.strip_prefix(cwd).map(|p| p.to_string_lossy().to_string()).unwrap_or_else(|_| path.to_string_lossy().to_string())
}

fn server_command(app: &App, command: ServerCommand, inventory_path: PathBuf) -> Result<(), Failure> {
    ...
    match command {
        ServerCommand::Run { idle_timeout } => {
            ...
            kapitan_server::run(inv, Duration::from_secs(idle_timeout), VERSION.to_string()).map_err(Failure::from)
        }
        ServerCommand::Start => {
        ...
        ServerCommand::Status => match Client::connect(&socket) {
            ...
            Err(_) => {
                if app.json {
                    ...
                    eprintln!("no server running for {} (socket {})", inventory_path.display(), socket.display());
                }
                Ok(())
            }
        },
...
fn print_info(info: &InfoResult, json: bool) {
    ...
    println!("  targets:    {} rendered, {} with errors", info.targets, info.errors);
    println!("  generation: {}", info.generation);
    println!("  uptime:     {}s (idle timeout {}s)", info.uptime_secs, info.idle_timeout_secs);
    if let Some(c) = &info.last_change {
        println!("  last change: gen {} re-rendered {} target(s) in {} ms", c.generation, c.rerendered.len(), c.duration_ms);
    }
}
