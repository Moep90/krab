...
//! Skipped unless `KAPITAN_CORPUS` (the JSON dir) and `KAPITAN_COMPILED`
...
#[test]
fn corpus_parity() {
    let (Ok(corpus), Ok(compiled)) = (
        std::env::var("KAPITAN_CORPUS"),
        std::env::var("KAPITAN_COMPILED"),
    ) else {
        eprintln!("KAPITAN_CORPUS / KAPITAN_COMPILED not set; skipping");
        return;
    };
    ...
    for f in &files {
        if f.file_name()
            .is_some_and(|n| n.to_string_lossy().starts_with(".kapitan-manifest"))
        {
            continue;
        }
