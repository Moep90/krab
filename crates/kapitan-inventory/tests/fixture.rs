...
fn renders_like_the_reference() {
    ...
    for entry in std::fs::read_dir(fixtures().join("expected")).unwrap() {
        ...
        let target = report.targets.get(&name).unwrap_or_else(|| panic!("target {name} not rendered"));
        ...
        if actual != expected {
            let diff: Vec<String> = expected
                ...
                .collect();
            panic!("target {name} differs from the reference output:\n{}", diff.join("\n"));
        }
        checked += 1;
    }
...
fn explain_reports_overrides() {
    ...
    let e = kapitan_inventory::explain::explain(&inv, &target, "database.engine").unwrap();
    assert_eq!(e.value.py_str(), "mysql");
    assert!(e.origin.as_ref().unwrap().file.ends_with("targets/env/prod.yml"));
    assert_eq!(e.history.len(), 1, "{:?}", e.history);
    let e = kapitan_inventory::explain::explain(&inv, &target, "database.hosts").unwrap();
    assert!(e.history.len() >= 2, "list appends recorded: {:?}", e.history);
    let e = kapitan_inventory::explain::explain(&inv, &target, "app.hostname").unwrap();
...
fn single_target_render_touches_only_its_closure() {
    ...
    let files: Vec<String> = target.files.iter().map(|f| f.file_name().unwrap().to_string_lossy().to_string()).collect();
    assert_eq!(files, vec!["bare.yml", "common.yml", "empty.yml"]);
    assert!(target.probes.iter().any(|p| p.ends_with("classes/common/init.yml")), "misses are recorded");
}
