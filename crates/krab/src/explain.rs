...
pub fn print_explanation(e: &Explanation) {
            ...
            match h {
                HistoryEntry::Override {
                    ...
                    let old = old_value
                        .as_ref()
                        .map(short)
                        .unwrap_or_else(|| format!("<{old_type}>"));
                    println!(
                        "    {} overrode {} from {}",
                        loc(new_location),
                        old,
                ...
                HistoryEntry::ListAppend { index, location } => {
                    println!("    {} appended item [{index}]", loc(location))
                }
                HistoryEntry::Dereference { expr, location } => {
                    println!(
                        "    {} placeholder {expr} expanded so a mapping could be merged into it",
                        loc(location)
                    )
                }
            }
    ...
    if e.children_events > 0 {
        println!(
            "  ({} more events under this path; explain a child path to see them)",
            e.children_events
        );
    }
}
