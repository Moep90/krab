...
impl<'input, T: Input> Scanner<'input, T> {
    ...
    pub fn fetch_next_token(&mut self) -> ScanResult {
        ...
        // kapitan: PyYAML does not enforce indentation inside flow collections
        // (`[` ... `]` spanning lines may close at the parent key's column).
    ...
    fn scan_block_scalar(&mut self, literal: bool) -> Result<Token<'input>, ScanError> {
        ...
        if chomping != Chomping::Strip {
            string.push_str(&leading_break);
            // kapitan: upstream synthesises a final newline when the stream ends without one;
            // PyYAML/libyaml do not, and the rendered inventory must match them.
        }
